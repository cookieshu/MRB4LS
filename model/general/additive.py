...
class AdditiveAttention(nn.Module):
    ...
    def __init__(self, attention_query_vector_dim, candidate_vector_dim):
        super().__init__()
        self.linear = nn.Linear(candidate_vector_dim,attention_query_vector_dim)
        self.attention_query_vector = nn.Parameter(torch.empty(attention_query_vector_dim).uniform_(-0.1, 0.1))
    ...
    def forward(self, candidate_vector, mask=None):
        ...
        target = torch.bmm(weights.unsqueeze(dim=1),candidate_vector).squeeze(dim=1)
        return target
